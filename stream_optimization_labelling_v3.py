...
#      n useful channels and class_type
...
plot_wiggles            = 0                                                  
trigger_verif_amt       = -20                                                    # how many plot_triggers to manually inspect (set to negative to ignore)
triggers_verified       = 0         
...
# globals for use in standard trigger detection function
std_on                  = -1
std_off                 = -1
...
def main():
    ...
    classes = ['cassm','meq','cassm', 'drilling', 'ert']
...
def return_trigger_index(wiggle, channel_name, class_type):
    ...
    for trigger_pair in trigger_indices:
        if not ( (trigger_pair[0]/len(ctf)) < depth_threshold ) and (trigger_pair[0]/len(ctf) < 0.7 ): # dont return triggers too close to the edge to min. padding
...
# Utility function to agnostically determine trigger locations of a given trace
...
# which corresponds to the peak values
...
#    b) class type (using global int32 class_dict mapping)
def optimize_class(class_type):
    global verified
    png_path = labeled_data_png_path + class_type + "\\*.png"
    
    if sequentially_load_pngs:                                                  # Usually set to 1 to find most amt of .mseeds
    ...
    class_label     = png_path.split("\\")[png_path.count("\\")-1]              # name of class eg 'ert', 'meq'..
    optimized_class_streams = []
    missed_pngs      = 0                                                        # counter for missing .mseed files
    handle_errors    = 0                                                        # boolean to deal with 'png not found' etc. errors
    error_msg        = []               
    ...
    for png in trange(max_pngs, leave=True):
        useful_traces    = []                                                   # List of traces which are representative of entire stream
        ...
        optimized_mseed_class_path += class_label + "_training\\"               # Directory of isolated training data
